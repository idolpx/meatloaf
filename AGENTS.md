...
- **The real fix is in ESP-IDF, and is applied by a build script** (`patch_framework.py`, new; wired into `[esp32_base]` as `pre:`): **esp-idf PR #18359 / IDFGH-17389** — `esp_http_client_prepare()` reset `data_process` and `first_line_prepared` but never reset the RESPONSE BUFFER, so `raw_data` stayed wherever the previous response stopped and a new request on the same handle parsed the previous response's leftovers as its own. That single defect explains every archive symptom over HTTPS in this session: buffers handed back unwritten (`0xBAAD5678` canaries, `.com`/`core` ASCII where a gzip trailer belongs), a gzip filter lost on re-open (a 174848-byte D64 measured as its 52223-byte compressed length), and a NULL-pointer `memcpy` inside `esp_http_client_read()`. The patch adds `esp_http_client_cached_buf_cleanup(client->response->buffer)` + `raw_len = 0` to `esp_http_client_prepare()`.
  ...
  **Still to do once this is verified on hardware**: the network-source guards added while hunting this — skipping the gzip ISIZE trailer probe (`canProbeGzipTrailer()`) and rebuilding the handle when a response cannot be drained — are workarounds for the same bug and can be reconsidered. Re-enabling the ISIZE probe would restore a known size for a `.gz` over HTTP (a listing currently shows 0).
