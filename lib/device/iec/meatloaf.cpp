...
{
//     // TODO IMPLEMENT
//     Debug_printf("iecMeatloaf::setup()\n");
 ...
 */
void iecMeatloaf::process_raw_cmd_data()
{
 ...
 */
void iecMeatloaf::process_immediate_raw_cmds()
{
...
void iecMeatloaf::get_status_basic()
{
...
void iecMeatloaf::unmount_host_basic()
{
    if (pt.size() < 2)
    {
...
void iecMeatloaf::unmount_host_raw()
{
...
bool iecMeatloaf::unmount_host(uint8_t hs)
{
    return _fnHosts[hs].umount();
}

void iecMeatloaf::mount_host_raw()
{
    int hs = payload[0];
...
void iecMeatloaf::mount_host_basic()
{
    if (pt.size() < 2)
    {
        response = "INVALID # OF PARAMETERS.";
...
void iecMeatloaf::disk_image_mount_basic()
{
    _populate_slots_from_config();
...
void iecMeatloaf::disk_image_mount_raw()
{
    _populate_slots_from_config();
...
void iecMeatloaf::set_boot_config_basic()
{
    if (pt.size() < 2)
    {
...
void iecMeatloaf::set_boot_config_raw()
{
    set_boot_config(payload[0] != 0);
    set_fuji_iec_status(0, "");
}

void iecMeatloaf::set_boot_config(bool should_boot_config)
{
...
// Set boot mode
void iecMeatloaf::set_boot_mode_basic()
{
    if (pt.size() < 2)
    {
...
void iecMeatloaf::set_boot_mode_raw()
{
    set_boot_mode(payload[0], true);
    set_fuji_iec_status(0, "");
}

void iecMeatloaf::set_boot_mode(uint8_t boot_device, bool should_boot_config)
{
...
*/
void iecMeatloaf::open_app_key_basic()
{
    unsigned int val;
...
void iecMeatloaf::open_app_key_raw()
{
...
void iecMeatloaf::open_app_key(uint16_t creator, uint8_t app, uint8_t key, appkey_mode mode, uint8_t reserved)
{
...
void iecMeatloaf::close_app_key_basic()
{
    close_app_key();
    response = "ok";
    set_fuji_iec_status(0, response);
...
void iecMeatloaf::close_app_key_raw()
{
    close_app_key();
    set_fuji_iec_status(0, "");
}
...
int iecMeatloaf::write_app_key(std::vector<uint8_t>&& value)
{
...
*/
void iecMeatloaf::read_app_key_basic()
{
...
void iecMeatloaf::read_app_key_raw()
{
    if (!fnSDFAT.running())
    {
        Debug_println("No SD mounted - can't read app key");
...
int iecMeatloaf::read_app_key(char *filename, std::vector<uint8_t>& file_data)
{
...
void iecMeatloaf::disk_image_umount_basic()
{
...
void iecMeatloaf::disk_image_umount_raw()
{
...
bool iecMeatloaf::disk_image_umount(uint8_t deviceSlot)
{
...
void iecMeatloaf::open_directory_basic()
{
...
void iecMeatloaf::open_directory_raw()
{
    Debug_println("Fuji cmd: OPEN DIRECTORY");
    if (payload.size() < 1)
    {
...
bool iecMeatloaf::validate_parameters_and_setup(uint8_t& maxlen, uint8_t& addtlopts) {
    if (pt.size() < 2) {
        return false;
    }
...
bool iecMeatloaf::validate_directory_slot() {
    return _current_open_directory_slot != -1;
}

void iecMeatloaf::read_directory_entry_basic() {
...
void iecMeatloaf::read_directory_entry_raw() {
    if (!validate_directory_slot()) {
...
void iecMeatloaf::get_directory_position_basic()
{
...
void iecMeatloaf::get_directory_position_raw()
{
    if (!validate_directory_slot())
    {
        set_fuji_iec_status(DEVICE_ERROR, "no currently open directory");
...
uint16_t iecMeatloaf::get_directory_position()
{
    return _fnHosts[_current_open_directory_slot].dir_tell();
}

void iecMeatloaf::set_directory_position_basic()
{
...
void iecMeatloaf::set_directory_position_raw()
{
    Debug_println("Fuji cmd: SET DIRECTORY POSITION");
...
bool iecMeatloaf::set_directory_position(uint16_t pos)
{
    return _fnHosts[_current_open_directory_slot].dir_seek(pos);
}

void iecMeatloaf::close_directory_basic()
{
    close_directory();
    response = "ok";
    set_fuji_iec_status(0, response);
...
void iecMeatloaf::close_directory_raw()
{
    close_directory();
    set_fuji_iec_status(0, "");
}
...
void iecMeatloaf::get_adapter_config_basic()
{
    get_adapter_config();
...
void iecMeatloaf::get_adapter_config_raw()
{
    get_adapter_config();
...
void iecMeatloaf::get_adapter_config_extended_raw()
{
...
AdapterConfigExtended iecMeatloaf::get_adapter_config_extended()
{
    // This reads the current configuration from the adapter into memory.
...
void iecMeatloaf::read_host_slots_basic()
{
...
void iecMeatloaf::read_host_slots_raw()
{
    Debug_println("Fuji cmd: READ HOST SLOTS");
...
void iecMeatloaf::write_host_slots_basic()
{
...
void iecMeatloaf::write_host_slots_raw()
{
...
void iecMeatloaf::read_device_slots_basic()
{
...
void iecMeatloaf::read_device_slots_raw()
{
...
void iecMeatloaf::write_device_slots_basic()
{
...
void iecMeatloaf::write_device_slots_raw()
{
    union _diskSlots
    {
        struct
...
void iecMeatloaf::set_device_filename_basic()
{
    if (pt.size() < 4)
    {
...
void iecMeatloaf::set_device_filename_raw()
{
...
void iecMeatloaf::set_device_filename(uint8_t slot, uint8_t host, uint8_t mode, std::string filename)
{
...
void iecMeatloaf::get_device_filename_basic()
{
...
void iecMeatloaf::get_device_filename_raw()
{
...
std::string iecMeatloaf::get_device_filename(uint8_t ds)
{
...
bool iecMeatloaf::mount_host(int hs)
{
    _populate_slots_from_config();
    return _fnHosts[hs].mount();
}

bool iecMeatloaf::disk_image_mount(uint8_t ds, uint8_t mode)
{
    char flag[3] = {'r', 0, 0};
...
bool iecMeatloaf::open_directory(uint8_t hs, std::string dirpath, std::string pattern)
{
...
std::string iecMeatloaf::process_directory_entry(uint8_t maxlen, uint8_t addtlopts) {
...
std::string iecMeatloaf::read_directory_entry(uint8_t maxlen, uint8_t addtlopts) {
