...
const char* psramfs_test_hello_str = "Hello, World!\n";
const char* psramfs_test_partition_label = "storage";
...
static const esp_partition_t *get_partition(void)
{
    const esp_partition_t *result = esp_partition_find_first(ESP_PARTITION_TYPE_DATA,
            ESP_PARTITION_SUBTYPE_DATA_SPIFFS, psramfs_test_partition_label);
    TEST_ASSERT_NOT_NULL_MESSAGE(result, "partition table not set correctly");
    return result;
}

static void test_psramfs_create_file_with_text(const char* name, const char* text)
{
...
static void test_psramfs_overwrite_append(const char* filename)
{
    /* Create new file with 'aaaa' */
    test_psramfs_create_file_with_text(filename, "aaaa");
    ...
    test_psramfs_create_file_with_text(filename, "cccc");
...
static void test_psramfs_read_file(const char* filename)
{
    FILE* f = fopen(filename, "r");
    TEST_ASSERT_NOT_NULL(f);
    char buf[32] = { 0 };
    int cb = fread(buf, 1, sizeof(buf), f);
    TEST_ASSERT_EQUAL(strlen(psramfs_test_hello_str), cb);
    TEST_ASSERT_EQUAL(0, strcmp(psramfs_test_hello_str, buf));
    TEST_ASSERT_EQUAL(0, fclose(f));
}

static void test_psramfs_open_max_files(const char* filename_prefix, size_t files_count)
{
...
static void test_psramfs_lseek(const char* filename)
{
...
static void test_psramfs_stat(const char* filename)
{
    test_psramfs_create_file_with_text(filename, "foo\n");
    struct stat st;
...
static void test_psramfs_unlink(const char* filename)
{
    test_psramfs_create_file_with_text(filename, "unlink\n");
...
static void test_psramfs_rename(const char* filename_prefix)
{
    char name_dst[64];
    char name_src[64];
...
static void test_psramfs_truncate(const char *filename)
{
    int read = 0;
    int truncated_len = 0;
    ...
    test_psramfs_create_file_with_text(filename, input);
...
static void test_psramfs_ftruncate(const char *filename)
{
    int truncated_len = 0;
...
static void test_psramfs_opendir_readdir_rewinddir(const char* dir_prefix)
{
    ...
    test_psramfs_create_file_with_text(name_dir_file1, "1\n");
    test_psramfs_create_file_with_text(name_dir_file2, "2\n");
    test_psramfs_create_file_with_text(name_dir_file3, "\01\02\03");
    test_psramfs_create_file_with_text(name_dir_inner_file, "3\n");
...
static void test_psramfs_readdir_many_files(const char* dir_prefix)
{
    ...
    char file_name[ESP_VFS_PATH_MAX + CONFIG_PSRAMFS_OBJ_NAME_LEN];
    ...
    for (int d = 0; d < n_folders; ++d) {
        ...
        for (int f = 0; f < n_files; ++f) {
            ...
            test_psramfs_create_file_with_text(file_name, file_name);
        }
    }
...
static void test_psramfs_concurrent(const char* filename_prefix)
{
    char names[4][64];
...
static void test_setup(void)
{
    esp_vfs_psramfs_conf_t conf = {
      .base_path = "/psram",
      .partition_label = psramfs_test_partition_label,
      .max_files = 5,
      .format_if_mount_failed = true
    };

    TEST_ESP_OK(esp_vfs_psramfs_register(&conf));
}

static void test_teardown(void)
{
    TEST_ESP_OK(esp_vfs_psramfs_unregister(psramfs_test_partition_label));
}

TEST_CASE("can initialize PSRAMFS in erased partition", "[psram]")
{
    ...
    TEST_ESP_OK(esp_psramfs_info(psramfs_test_partition_label, &total, &used));
    printf("total: %d, used: %d\n", total, used);
    TEST_ASSERT_EQUAL(0, used);
    test_teardown();
}

TEST_CASE("can format mounted partition", "[psram]")
{
    // Mount PSRAMFS, create file, format, check that the file does not exist.
    const esp_partition_t* part = get_partition();
    TEST_ASSERT_NOT_NULL(part);
    test_setup();
    const char* filename = "/psram/hello.txt";
    test_psramfs_create_file_with_text(filename, psramfs_test_hello_str);
    esp_psramfs_format(part->label);
    FILE* f = fopen(filename, "r");
    TEST_ASSERT_NULL(f);
    test_teardown();
}

TEST_CASE("can format unmounted partition", "[psram]")
{
    // Mount PSRAMFS, create file, unmount. Format. Mount again, check that
...
TEST_CASE("can create and write file", "[psram]")
{
    test_setup();
    test_psramfs_create_file_with_text("/psram/hello.txt", psramfs_test_hello_str);
    test_teardown();
}

TEST_CASE("can read file", "[psram]")
{
    test_setup();
    test_psramfs_create_file_with_text("/psram/hello.txt", psramfs_test_hello_str);
    test_psramfs_read_file("/psram/hello.txt");
    test_teardown();
}

TEST_CASE("can open maximum number of files", "[psram]")
{
    ...
    TEST_ESP_OK(esp_vfs_psramfs_register(&conf));
    test_psramfs_open_max_files("/psram/f", max_files);
    TEST_ESP_OK(esp_vfs_psramfs_unregister(psramfs_test_partition_label));
}

TEST_CASE("overwrite and append file", "[psram]")
{
    test_setup();
    test_psramfs_overwrite_append("/psram/hello.txt");
    test_teardown();
}

TEST_CASE("can lseek", "[psram]")
{
    test_setup();
    test_psramfs_lseek("/psram/seek.txt");
    test_teardown();
}


TEST_CASE("stat returns correct values", "[psram]")
{
    test_setup();
    test_psramfs_stat("/psram/stat.txt");
    test_teardown();
}

TEST_CASE("unlink removes a file", "[psram]")
{
    test_setup();
    test_psramfs_unlink("/psram/unlink.txt");
    test_teardown();
}

TEST_CASE("rename moves a file", "[psram]")
{
    test_setup();
    test_psramfs_rename("/psram/move");
    test_teardown();
}

TEST_CASE("truncate a file", "[psram]")
{
    test_setup();
    test_psramfs_truncate("/psram/truncate.txt");
    test_teardown();
}

TEST_CASE("ftruncate a file", "[psram]")
{
    test_setup();
    test_psramfs_ftruncate("/psram/ftrunc.txt");
    test_teardown();
}

TEST_CASE("can opendir root directory of FS", "[psram]")
{
    test_setup();
    test_psramfs_can_opendir("/psram");
    test_teardown();
}

TEST_CASE("opendir, readdir, rewinddir, seekdir work as expected", "[psram]")
{
    test_setup();
    test_psramfs_opendir_readdir_rewinddir("/psram/dir");
    test_teardown();
}

TEST_CASE("readdir with large number of files", "[psram][timeout=30]")
{
    test_setup();
    test_psramfs_readdir_many_files("/psram/dir2");
    test_teardown();
}

TEST_CASE("multiple tasks can use same volume", "[psram]")
{
    test_setup();
    test_psramfs_concurrent("/psram/f");
    test_teardown();
}

#ifdef CONFIG_PSRAMFS_USE_MTIME
TEST_CASE("mtime is updated when file is opened", "[psram]")
{
    /* Open a file, check that mtime is set correctly */
    const char* filename = "/psram/time";
    test_setup();
    time_t t_before_create = time(NULL);
    test_psramfs_create_file_with_text(filename, "\n");
...
TEST_CASE("utime() works well", "[psram]")
{
    const char filename[] = "/psram/utime.txt";
...
static void test_psramfs_rw_speed(const char* filename, void* buf, size_t buf_size, size_t file_size, bool is_write)
{
...
TEST_CASE("write/read speed test", "[psram][timeout=60]")
{
    ...
    const char* file = "/psram/speedtest.bin";

    test_psramfs_rw_speed(file, buf, 4 * 1024, file_size, true);
    TEST_ASSERT_EQUAL(0, unlink(file));
    TEST_ESP_OK(esp_psramfs_gc(psramfs_test_partition_label, file_size));

    test_psramfs_rw_speed(file, buf, 8 * 1024, file_size, true);
    TEST_ASSERT_EQUAL(0, unlink(file));
    TEST_ESP_OK(esp_psramfs_gc(psramfs_test_partition_label, file_size));

    test_psramfs_rw_speed(file, buf, 16 * 1024, file_size, true);

    test_psramfs_rw_speed(file, buf, 4 * 1024, file_size, false);
    test_psramfs_rw_speed(file, buf, 8 * 1024, file_size, false);
    test_psramfs_rw_speed(file, buf, 16 * 1024, file_size, false);
    TEST_ASSERT_EQUAL(0, unlink(file));
    TEST_ESP_OK(esp_psramfs_gc(psramfs_test_partition_label, file_size));
...
TEST_CASE("PSRAMFS garbage-collect", "[psram][timeout=60]")
{
    // should fail until the partition is initialized
    TEST_ESP_ERR(ESP_ERR_INVALID_STATE, esp_psramfs_gc(psramfs_test_partition_label, 4096));

    test_setup();

    // reclaiming one block should be possible
    TEST_ESP_OK(esp_psramfs_gc(psramfs_test_partition_label, 4096));

    // shouldn't be possible to reclaim more than the partition size
    const esp_partition_t* part = get_partition();
    TEST_ESP_ERR(ESP_ERR_NOT_FINISHED, esp_psramfs_gc(psramfs_test_partition_label, part->size * 2));
