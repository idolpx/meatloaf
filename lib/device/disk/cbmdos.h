...
class CBMDOS: public DOSCOMMAND
{
...
struct cbmdos_cmd_parse_s {
    ...
    size_t cmdlength; /* input */
    char *parsecmd; /* output: parsed command */
    size_t secondary; /* input */
    size_t parselength; /* output */
    size_t readmode; /* output */
    size_t filetype; /* output */
    size_t recordlength; /* output */
    int drive; /* output: drive number */
    size_t atsign; /* @0:filename, detected in fsdevice_open() */
};
...
struct cbmdos_cmd_parse_plus_s {
    ...
    size_t fulllength; /* input: length of string */
    size_t secondary; /* input */
    ...
    size_t abbrvlength; /* abbreviated command length */
    ...
    size_t morelength; /* length of command to further process */

    /* for file mode */
    size_t readmode; /* output */
    size_t filetype; /* output */
    size_t recordlength; /* output */
    size_t colon; /* output, 0=no colon found, !0=colon found */
};
typedef struct cbmdos_cmd_parse_plus_s cbmdos_cmd_parse_plus_t;

const char *cbmdos_errortext(size_t code);
const char *cbmdos_filetype_get(size_t filetype);

size_t cbmdos_parse_wildcard_check(const char *name, size_t len);
size_t cbmdos_parse_wildcard_compare(const uint8_t *name1, const uint8_t *name2);
uint8_t *cbmdos_dir_slot_create(const char *name, size_t len);

size_t cbmdos_command_parse(cbmdos_cmd_parse_t *cmd_parse);
size_t cbmdos_command_parse_plus(cbmdos_cmd_parse_plus_t *cmd_parse);
