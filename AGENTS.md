...
- **Hardware-verified on lolin-d32-pro**, console over serial and over TCP:
  ...
  - Block windows, on a `"#"` channel over `goonies.d64`: `open` gives SIZE 256 POS 0 and `read`
    ...
    from the block base and not from B-R's byte-1 start. `F-P 2 91653` re-bases mid-block: POS 5,
    251 bytes to the block end. On a FILE channel `B-P` answers `31,INVALID COMMAND` while
    `F-P 3 254` seeks absolutely and reads `59 1A 4A 16 09 BF 90 A9`, matching the linear dump. Bad track and bad sector both
