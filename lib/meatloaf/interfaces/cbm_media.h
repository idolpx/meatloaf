...
#include "../interfaces/seekable.h"
#include "../interfaces/is_randomaccess.h"

/********************************************************
 * Streams
 ********************************************************/

class CBMDisk: public Seekable, IsRandomAccess {

public:
    CBMDisk(std::shared_ptr<MStream> is) {
        containerStream = is;
        _is_open = true;
        has_subdirs = false;
    }
 ...
 ********************************************************/
class ImageBroker {
    static std::unordered_map<std::string, CBMDisk*> repo;
public:
    ...
    static CBMDisk* obtain(std::string url) {
        return obtain<CBMDisk>(url);
    }
