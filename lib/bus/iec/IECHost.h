...
class IECHost
{
 ...
 public:
  ...
  // errors may result
  IECHost(uint8_t devnr = 0xFF);

  // call this to change the device number
  void setDeviceNumber(uint8_t devnr);

  // call this to check if a device exists on the bus
 ...
 protected:
  ...
  // the default implementation within IECHost uses the canWrite() and write(data,eoi) functions,
  ...
  // read() is allowed to take an indefinite amount of time
  // the default implementation within IECHost uses the canRead() and read() functions,
  // which is not efficient.
