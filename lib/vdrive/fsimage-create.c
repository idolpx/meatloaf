...
static int fsimage_create_dxx(disk_image_t *image)
{
    ...
    if (image->type == DISK_IMAGE_TYPE_X64) {
        ...
        if (archdep_fwrite(header, X64_HEADER_LENGTH, 1, fsimage->fd) < 1) {
            log_error(createdisk_log,
    ...
    for (i = 0; i < (size / 256); i++) {
        if (archdep_fwrite(block, 256, 1, fsimage->fd) < 1) {
            log_error(createdisk_log,
...
int fsimage_create_dxm(const char *name, const char *diskname, unsigned int type)
{
    ...
    fd = archdep_fopen(name, MODE_WRITE);
    ...
    for (i = 0; i < size / 256; i++) {
        ...
        if (i == 1) {
            ...
            if (archdep_fwrite(block, 256, 1, fd) < 1) {
                log_error(createdisk_log,
                          "Cannot seek to end of disk image `%s'.",
                          name);
                rc = -1;
...
static int fsimage_create_gcr(disk_image_t *image)
{
    ...
    if (archdep_fwrite(gcr_header, sizeof(gcr_header), 1, fsimage->fd) < 1) {
    ...
    if (archdep_fwrite(gcr_track_p, (max_tracks * 2 * 4), 1, fsimage->fd) < 1) {
        log_error(createdisk_log, "Cannot write track header.");
        return -1;
    }
    /* write speed table */
    if (archdep_fwrite(gcr_speed_p, (max_tracks * 2 * 4), 1, fsimage->fd) < 1) {
    ...
    for (track = 1; track <= num_tracks; track++) {
        ...
        DBG(("(gap: %d)\n", gap));
        if (archdep_fwrite((char *)gcr_track, sizeof(gcr_track), 1, fsimage->fd) < 1) {
...
static int fsimage_create_p64(disk_image_t *image)
{
    ...
    if (P64ImageWriteToStream(&P64Image, &P64MemoryStreamInstance)) {
        if (archdep_fwrite(P64MemoryStreamInstance.Data, P64MemoryStreamInstance.Size, 1, fsimage->fd) < 1) {
...
int fsimage_create(const char *name, unsigned int type)
{
    ...
    fsimage->fd = archdep_fopen(name, MODE_WRITE);
