...
- **Explicit FTPS (RFC 4217) is negotiated OPPORTUNISTICALLY, on a retry, and never on the first attempt.** `fnFTP::login()` connects in the clear; only when the server refuses a command with a 4xx/5xx whose text names TLS or SSL (`_tls_required`, set at the banner, at USER and at PASS — servers differ on which one they refuse) does it reconnect and send `AUTH TLS`. So a plaintext server pays nothing, and no URL scheme, config key or build flag exists or is needed — `ftps://` was deliberately not added. The retry is capped at one: `login()` returns `do_login(true)`'s result directly.
