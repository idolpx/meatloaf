...
static s32_t _read(
  ...
  if (log_flash_ops) {
    ...
    if (error_after_bytes_read > 0 && bytes_rd >= error_after_bytes_read) {
      ...
      return PSRAMFS_ERR_TEST;
    }
  }
  if (addr < PSRAMFS_CFG_PHYS_ADDR(&__fs)) {
    printf("FATAL read addr too low %08x < %08x\n", addr, PSRAMFS_PHYS_ADDR);
    ERREXIT();
    return -1;
  }
  if (addr + size > PSRAMFS_CFG_PHYS_ADDR(&__fs) + PSRAMFS_CFG_PHYS_SZ(&__fs)) {
    printf("FATAL read addr too high %08x + %08x > %08x\n", addr, size, PSRAMFS_PHYS_ADDR + PSRAMFS_FLASH_SIZE);
    ERREXIT();
    return -1;
  }
...
static s32_t _write(
  ...
  for (i = 0; i < size; i++) {
    ...
    if (((addr + i) & (PSRAMFS_CFG_LOG_PAGE_SZ(&__fs)-1)) == offsetof(psramfs_page_header, flags)) {
    ...
    if (check_valid_flash && ((AREA(addr + i) ^ src[i]) & src[i])) {
      ...
      psramfs_page_ix pix = (addr + i) / PSRAMFS_CFG_LOG_PAGE_SZ(&__fs);
      dump_page(&__fs, pix);
      ERREXIT();
      return -1;
    }
...
static s32_t _erase(
  ...
  if (addr & (PSRAMFS_CFG_PHYS_ERASE_SZ(&__fs)-1)) {
    printf("trying to erase at addr %08x, out of boundary\n", addr);
    ERREXIT();
    return -1;
  }
  if (size & (PSRAMFS_CFG_PHYS_ERASE_SZ(&__fs)-1)) {
    printf("trying to erase at with size %08x, out of boundary\n", size);
    ERREXIT();
    return -1;
  }
  _erases[(addr-PSRAMFS_CFG_PHYS_ADDR(&__fs))/PSRAMFS_CFG_PHYS_ERASE_SZ(&__fs)]++;
...
void dump_page(psram *fs, psramfs_page_ix p) {
  printf("page %04x  ", p);
  u32_t addr = PSRAMFS_PAGE_TO_PADDR(fs, p);
  if (p % PSRAMFS_PAGES_PER_BLOCK(fs) < PSRAMFS_OBJ_LOOKUP_PAGES(fs)) {
    // obj lu page
    printf("OBJ_LU");
  } else {
    u32_t obj_id_addr = PSRAMFS_BLOCK_TO_PADDR(fs, PSRAMFS_BLOCK_FOR_PAGE(fs , p)) +
        PSRAMFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, p) * sizeof(psramfs_obj_id);
    psramfs_obj_id obj_id = *((psramfs_obj_id *)&AREA(obj_id_addr));
    // data page
    psramfs_page_header *ph = (psramfs_page_header *)&AREA(addr);
    printf("DATA %04x:%04x  ", obj_id, ph->span_ix);
    printf("%s", ((ph->flags & PSRAMFS_PH_FLAG_FINAL) == 0) ? "FIN " : "fin ");
    printf("%s", ((ph->flags & PSRAMFS_PH_FLAG_DELET) == 0) ? "DEL " : "del ");
    printf("%s", ((ph->flags & PSRAMFS_PH_FLAG_INDEX) == 0) ? "IDX " : "idx ");
    printf("%s", ((ph->flags & PSRAMFS_PH_FLAG_USED) == 0) ? "USD " : "usd ");
    printf("%s  ", ((ph->flags & PSRAMFS_PH_FLAG_IXDELE) == 0) ? "IDL " : "idl ");
    if (obj_id & PSRAMFS_OBJ_ID_IX_FLAG) {
      // object index
      printf("OBJ_IX");
      if (ph->span_ix == 0) {
        printf("_HDR  ");
        psramfs_page_object_ix_header *oix_hdr = (psramfs_page_object_ix_header *)&AREA(addr);
  ...
  printf("\n");
  u32_t len = PSRAMFS_CFG_LOG_PAGE_SZ(fs);
  hexdump(addr, len);
}
...
void dump_erase_counts(psram *fs) {
  psramfs_block_ix bix;
  psramfs_block_ix bix_offs;
  printf("  BLOCK     |\n");
  printf("   AGE COUNT|\n");
  for (bix_offs = 0; bix_offs < fs->block_count; bix_offs+=8) {
    ...
    printf("\n");
    for (bix = bix_offs; bix < bix_offs+8 && bix < fs->block_count; bix++) {
      psramfs_obj_id erase_mark;
      _psramfs_rd(fs, 0, 0, PSRAMFS_ERASE_COUNT_PADDR(fs, bix), sizeof(psramfs_obj_id), (u8_t *)&erase_mark);
      if (_erases[bix] == 0) {
        printf("            |");
      } else {
...
static void psramfs_check_cb_f(
#if PSRAMFS_HAL_CALLBACK_EXTRA
    psram *fs,
#endif
    psramfs_check_type type, psramfs_check_report report,
    u32_t arg1, u32_t arg2) {
/*  if (report == PSRAMFS_CHECK_PROGRESS && old_perc != arg1) {
    old_perc = arg1;
    printf("CHECK REPORT: ");
    switch(type) {
    case PSRAMFS_CHECK_LOOKUP:
      printf("LU "); break;
    case PSRAMFS_CHECK_INDEX:
      printf("IX "); break;
    case PSRAMFS_CHECK_PAGE:
      printf("PA "); break;
    }
    printf("%i%%\n", arg1 * 100 / 256);
  }*/
  if (report != PSRAMFS_CHECK_PROGRESS) {
    check_cb_count++;
    if (report != PSRAMFS_CHECK_ERROR) fs_check_fixes++;
    printf("   check: ");
    switch (type) {
    case PSRAMFS_CHECK_INDEX:
      printf("INDEX  "); break;
    ...
    if (report == PSRAMFS_CHECK_ERROR) {
      printf("ERROR %i", arg1);
    } else if (report == PSRAMFS_CHECK_DELETE_BAD_FILE) {
      printf("DELETE BAD FILE %04x", arg1);
    } else if (report == PSRAMFS_CHECK_DELETE_ORPHANED_INDEX) {
      printf("DELETE ORPHANED INDEX %04x", arg1);
    } else if (report == PSRAMFS_CHECK_DELETE_PAGE) {
      printf("DELETE PAGE %04x", arg1);
    } else if (report == PSRAMFS_CHECK_FIX_INDEX) {
      printf("FIX INDEX %04x:%04x", arg1, arg2);
    } else if (report == PSRAMFS_CHECK_FIX_LOOKUP) {
      printf("FIX INDEX %04x:%04x", arg1, arg2);
    } else {
      printf("??");
    }
...
s32_t fs_mount_specific(u32_t phys_addr, u32_t phys_size,
  ...
  c.fh_ix_offset = TEST_PSRAMFS_FILEHDL_OFFSET;
#endif
  return PSRAMFS_mount(&__fs, &c, _work, _fds, _fds_sz, _cache, _cache_sz, psramfs_check_cb_f);
}
...
static void fs_create(u32_t spiflash_size,
  ...
  _fds_sz = descriptors * sizeof(psramfs_fd);
  _fds = malloc(_fds_sz);
  ASSERT(_fds != NULL, "testbench fd buffer could not be malloced");
  memset(_fds, 0, _fds_sz);

#if PSRAMFS_CACHE
  _cache_sz = sizeof(psramfs_cache) + cache_pages * (sizeof(psramfs_cache_page) + log_page_size);
...
void fs_reset_specific(u32_t addr_offset, u32_t phys_addr, u32_t phys_size,
  ...
  if (res == PSRAMFS_OK) {
    PSRAMFS_unmount(&__fs);
  }
  res = PSRAMFS_format(&__fs);
  if (res != PSRAMFS_OK) {
...
void fs_reset() {
  fs_reset_specific(0, PSRAMFS_PHYS_ADDR, PSRAMFS_FLASH_SIZE, SECTOR_SIZE, LOG_BLOCK, LOG_PAGE);
}
...
void fs_mount_dump(char *fname,
  ...
  ASSERT(res == PSRAMFS_OK, "failed mounting dump, check settings");
...
int read_and_verify(char *name) {
  int fd = PSRAMFS_open(&__fs, name, PSRAMFS_RDONLY, 0);
  if (fd < 0) {
...
int read_and_verify_fd(psramfs_file fd, char *name) {
  s32_t res;
  int pfd = open(make_test_fname(name), O_RDONLY);
  psramfs_stat s;
  res = PSRAMFS_fstat(&__fs, fd, &s);
  if (res < 0) {
  ...
  if (s.size != fsize) {
    printf("  read_and_verify: size differs, %s psram:%d!=fs:%ld\n", name, s.size, fsize);
    return -1;
  }
  lseek(pfd, 0, SEEK_SET);

  if (s.size == 0) {
    PSRAMFS_close(&__fs, fd);
    close(pfd);
    return 0;
  }
  ...
  while (offs < s.size) {
    ...
    res = PSRAMFS_read(&__fs, fd, buf_d, read_len);
    if (res < 0) {
    ...
    (void)pres;
    //printf("reading offs:%i len:%i psramfs_res:%i posix_res:%i\n", offs, read_len, res, pres);
    int i;
    int veri_ok = 1;
...
static void test_on_stop(test *t) {
  printf("  psram errno:%i\n", PSRAMFS_errno(&__fs));
#if PSRAMFS_TEST_VISUALISATION
  if (_area) PSRAMFS_vis(FS);
#endif
...
int test_create_file(char *name) {
  psramfs_stat s;
  psramfs_file fd;
  int res = PSRAMFS_creat(FS, name, 0);
  CHECK_RES(res);
...
u32_t get_psramfs_file_crc_by_fd(psramfs_file fd) {
  s32_t res;
  u32_t crc = 0;
  u8_t buf[256];

  ASSERT(PSRAMFS_lseek(FS, fd, 0, PSRAMFS_SEEK_SET) >= 0, "could not seek to start of file");

  while ((res = PSRAMFS_read(FS, fd, buf, sizeof(buf))) > PSRAMFS_OK) {
    crc = crc32(crc, buf, res);
  }
  ASSERT(PSRAMFS_errno(FS) == PSRAMFS_ERR_END_OF_OBJECT || PSRAMFS_errno(FS) == PSRAMFS_OK, "failed reading file");

  return crc;
}

u32_t get_psramfs_file_crc(char *name) {
  s32_t res;
  psramfs_file fd;
  fd = PSRAMFS_open(FS, name, PSRAMFS_O_RDONLY, 0);
  ASSERT(fd >= 0, "Could not open file");
  u32_t crc = get_psramfs_file_crc_by_fd(fd);
  res = PSRAMFS_close(FS, fd);
  ASSERT(res >= PSRAMFS_OK, "failing closing file");
  return crc;
}
...
u32_t tfile_get_size(tfile_size s) {
  ...
  case SMALL: // half a data page
    return PSRAMFS_DATA_PAGE_SIZE(FS)/2;
  case MEDIUM: // one block
    return PSRAMFS_DATA_PAGE_SIZE(FS) * (PSRAMFS_PAGES_PER_BLOCK(FS) - PSRAMFS_OBJ_LOOKUP_PAGES(FS));
  case LARGE: // third of fs
    return PSRAMFS_DATA_PAGE_SIZE(FS) * (PSRAMFS_PAGES_PER_BLOCK(FS) - PSRAMFS_OBJ_LOOKUP_PAGES(FS)) * (FS)->block_count/3;
  }
  return 0;
}
...
// create a new file
        ...
        if (tf->cfg.tsize == EMPTY) {
          res = PSRAMFS_creat(FS, name, 0);
          CHECK_RES(res);
          int pfd = open(make_test_fname(name), O_TRUNC | O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
          close(pfd);
          int extra_flags = tf->cfg.ttype == APPENDED ? PSRAMFS_APPEND : 0;
          psramfs_file fd = PSRAMFS_open(FS, name, extra_flags | PSRAMFS_RDWR, 0);
          CHECK(fd > 0);
          tf->fd = fd;
        } else {
          int extra_flags = tf->cfg.ttype == APPENDED ? PSRAMFS_APPEND : 0;
          psramfs_file fd = PSRAMFS_open(FS, name, extra_flags | PSRAMFS_TRUNC | PSRAMFS_CREAT | PSRAMFS_RDWR, 0);
          CHECK(fd > 0);
...
// hande file lifecycle
        ...
        case APPENDED: {
          ...
          u8_t *buf = malloc(size);
          memrand(buf, size);
          res = PSRAMFS_write(FS, tf->fd, buf, size);
          CHECK_RES(res);
        ...
        case MODIFIED: {
          ...
          psramfs_stat stat;
          res = PSRAMFS_fstat(FS, tf->fd, &stat);
          CHECK_RES(res);
          int size = stat.size / tf->cfg.tlife + PSRAMFS_DATA_PAGE_SIZE(FS)/3;
          int offs = (stat.size / tf->cfg.tlife) * tf->state;
          res = PSRAMFS_lseek(FS, tf->fd, offs, PSRAMFS_SEEK_SET);
          CHECK_RES(res);
          u8_t *buf = malloc(size);
          memrand(buf, size);
          res = PSRAMFS_write(FS, tf->fd, buf, size);
        ...
        case REWRITTEN: {
          if (tf->fd > 0) {
            PSRAMFS_close(FS, tf->fd);
          }
          if (dbg) printf("   rewriting %s\n", tf->name);
          psramfs_file fd = PSRAMFS_open(FS, tf->name, PSRAMFS_TRUNC | PSRAMFS_CREAT | PSRAMFS_RDWR, 0);
          CHECK(fd > 0);
