...
uint8_t speed_map_1541[MAX_TRACKS_1541] = {
...
static uint8_t GCR_conv_data[16] = {
	0x0a, 0x0b, 0x12, 0x13,
	0x0e, 0x0f, 0x16, 0x17,
	0x09, 0x19, 0x1a, 0x1b,
	0x0d, 0x1d, 0x1e, 0x15
...
int
find_sync(uint8_t ** gcr_pptr, uint8_t * gcr_end)
{
	while (1)
	{
...
void
convert_4bytes_to_GCR(uint8_t * buffer, uint8_t * ptr)
{
...
int
convert_4bytes_from_GCR(uint8_t * gcr, uint8_t * plain)
{
...
int
extract_id(uint8_t * gcr_track, uint8_t * id)
{
	uint8_t header[10];
	uint8_t *gcr_ptr, *gcr_end;
	int track, sector;
...
int
extract_cosmetic_id(uint8_t * gcr_track, uint8_t * id)
{
	uint8_t secbuf[260];
	int error;
...
uint8_t
convert_GCR_sector(uint8_t * gcr_start, uint8_t * gcr_cycle, uint8_t * d64_sector,
  int track, int sector, uint8_t * id)
{
	uint8_t header[10];	/* block header */
	uint8_t hdr_chksum;	/* header checksum */
	uint8_t blk_chksum;	/* block  checksum */
	uint8_t gcr_buffer[2 * NIB_TRACK_LENGTH];
	uint8_t *gcr_ptr, *gcr_end, *gcr_last;
	uint8_t *sectordata;
	uint8_t error_code;
...
void
convert_sector_to_GCR(uint8_t * buffer, uint8_t * ptr,
...
size_t
find_track_cycle(uint8_t ** cycle_start, uint8_t ** cycle_stop, int cap_min, int cap_max)
{
	...
	uint8_t *data_pos;		/* cycle search variable */
...
size_t
find_nondos_track_cycle(uint8_t ** cycle_start, uint8_t ** cycle_stop, int cap_min,
  int cap_max)
{
	uint8_t *nib_track;	/* start of nibbled track data */
	uint8_t *start_pos;	/* start of periodic area */
...
int
check_valid_data(uint8_t * data, int matchlen)
{
...
uint8_t *
find_sector0(uint8_t * work_buffer, int tracklen, size_t * p_sectorlen)
{
	uint8_t *pos, *buffer_end, *sync_last;
...
uint8_t *
find_sector_gap(uint8_t * work_buffer, int tracklen, size_t * p_sectorlen)
{
	size_t gap, maxgap;
	uint8_t *pos;
	uint8_t *buffer_end;
...
int
check_formatted(uint8_t * gcrdata)
{
	int i, run = 0;
 ...
 */
int
extract_GCR_track(uint8_t * destination, uint8_t * source, int * align,
  int force_align, size_t cap_min, size_t cap_max)
{
	uint8_t work_buffer[NIB_TRACK_LENGTH*2];	/* working buffer */
	...
	uint8_t *longsync_pos;	/* position of longest sync run */
	uint8_t *weakgap_pos;	/* position of weak bit run */
	uint8_t *marker_pos;	/* generic marker used by protection handlers */
 ...
 */
int
strip_runs(uint8_t * buffer, int length, int minrun, uint8_t target)
{
	int run, skipped;
	uint8_t *source, *end;
...
int
reduce_runs(uint8_t * buffer, int length, int length_max, int minrun,
  uint8_t target)
{
	int skipped;
...
int
check_sync_flags(uint8_t * gcrdata, int density, int length)
{
	int i, syncs = 0;
...
int
compare_tracks(uint8_t * track1, uint8_t * track2, int length1, int length2,
...
int
check_errors(uint8_t * gcrdata, int length, int track, uint8_t * id, char * errorstring)
{
...
int
check_empty(uint8_t * gcrdata, int length, int track, uint8_t * id, char * errorstring)
{
 ...
 */
int
replace_bytes(uint8_t * buffer, int length, uint8_t srcbyte, uint8_t dstbyte)
{
	int i, replaced;
...
int
is_bad_gcr(uint8_t * gcrdata, size_t length, size_t pos)
{
 ...
 */
int
check_bad_gcr(uint8_t * gcrdata, int length, int fix)
{
