#!/usr/bin/env python3
"""Encode a .d64 into a .g64 so the G64 read path has something to test against.
...
def main():
    ...
    out = bytearray(b"GCR-1541")
    out += bytes([0])                                # version
    out += bytes([84])                               # half track count
    ...
    # The tables are 84 entries each; data starts after both of them.
    base = len(out) + (84 * 4) + (84 * 4)

    for half in range(84):
        if half % 2 or (half // 2) >= 35:
