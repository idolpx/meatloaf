...
class SpiffsgenTest(unittest.TestCase):
    def test_configs(self):  # type: () -> None
        """Run psramfsgen with different configs, and check that
        an image is generated (there is no exception), and the image size
        is as expected.
        """
        default_config = dict(
            page_size=256,
            page_ix_len=psramfsgen.PSRAMFS_PAGE_IX_LEN,
            block_size=4096,
            block_ix_len=psramfsgen.PSRAMFS_BLOCK_IX_LEN,
            meta_len=4,
            obj_name_len=32,
            obj_id_len=psramfsgen.PSRAMFS_BLOCK_IX_LEN,
            span_ix_len=psramfsgen.PSRAMFS_SPAN_IX_LEN,
            packed=True,
            aligned=True,
            endianness='little',
            use_magic=True,
        ...
        def make_config(**kwargs):  # type: (typing.Any) -> psramfsgen.SpiffsBuildConfig
            ...
            return psramfsgen.SpiffsBuildConfig(**new_config)
        ...
        for config in configs:
            psram = psramfsgen.SpiffsFS(image_size, config)
            psram.create_file('/test', __file__)
            image = psram.to_binary()
            self.assertEqual(len(image), image_size)
            # Note: it would be nice to compile psram for host with the given
