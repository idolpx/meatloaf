...
const char *CBMDOS::cbmdos_errortext(size_t code)
{
    size_t count = 0;
...
const char *CBMDOS::cbmdos_filetype_get(size_t filetype)
{
    return cbmdos_ft[filetype];
}

size_t CBMDOS::cbmdos_parse_wildcard_check(const char *name, size_t len)
{
    size_t index;
...
size_t CBMDOS::cbmdos_parse_wildcard_compare(const uint8_t *name1, const uint8_t *name2)
{
    size_t index;
...
uint8_t *CBMDOS::cbmdos_dir_slot_create(const char *name, size_t len)
{
    uint8_t *slot;
...
size_t CBMDOS::cbmdos_command_parse(cbmdos_cmd_parse_t *cmd_parse)
{
    const uint8_t *p;
    char *parsecmd, *c;
    int cmdlen;
...
size_t CBMDOS::cbmdos_command_parse_plus(cbmdos_cmd_parse_plus_t *cmd_parse)
{
    ...
    if ((cmd_parse->mode == 0 || cmd_parse->mode == 2) && p < limit) {
        ...
        if (p2 || special) {
            ...
            if (*p != '/' && (*p < '0' || *p > '9')) {
                ...
                cmd_parse->commandlength = (size_t)(p - p1);
...
/* TODO: CMD limits paths by having them begin and end with a '/' */
                ...
                if (p < p2) {
                    p1 = p;
                    p = p2;
                    cmd_parse->pathlength = (size_t)(p - p1);
        ...
        cmd_parse->filelength = (size_t)(p - p1);
    ...
    if (cmd_parse->mode == 1 && p < limit) {
        ...
        if (p < limit) {
            cmd_parse->morelength = (size_t)(limit - p);
