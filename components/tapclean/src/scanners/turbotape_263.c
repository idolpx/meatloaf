/*
 * turbotape_263.c (written by Luigi Di Fraia, Feb 2023)
 * Based on turbotape_526.c
 *
...
/*
 * Description: Rip of the Turbo Tape 250 "clone" by Schmendrick der Zauberer,
 *              used in "Wladca" by Mirage (Polish): LSbF instead of MSbF.
 */
		...
		if (eop > 0) {
			...
			switch (ftype) {
				...
				case FILE_TYPE_HEADER_NON_RELOC:
					...
					if (addblockdefex(TT263_HEAD, sof, sod, eod, eof, xinfo, ftype) >= 0) {
				...
				case FILE_TYPE_DATA:	/* Data */
					...
					if (addblockdefex(TT263_DATA, sof, sod, eod, eof, xinfo, ftype) >= 0)
						i = eof;	/* Search for further files starting from the end of this one */
...
int turbotape263_describe(int row)
{
	int i, s;
	int ftype, lt;
	int en, tp, sp, lp;
