...
// //std::unordered_map<std::string, CBMDisk*> ImageBroker::repo;

// // Utility Functions

// std::string CBMDisk::decodeType(uint8_t file_type, bool show_hidden)
// {
//     //bool hide = false;
...
// // std::string CBMDisk::seekNextEntry() {
...
// uint32_t CBMDisk::seekFileSize( uint8_t start_track, uint8_t start_sector )
// {
//     // Calculate file size
...
// uint16_t CBMDisk::readContainer(uint8_t *buf, uint16_t size)
// {
//     return containerStream->read(buf, size);
// }


// uint32_t CBMDisk::read(uint8_t* buf, uint32_t size) {
...
// uint32_t CBMDisk::write(const uint8_t *buf, uint32_t size) {
//     return -1;
// }
