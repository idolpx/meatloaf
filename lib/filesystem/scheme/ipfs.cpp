...
MFile* IPFSFile::getNextFileInDir() {
	...
	if(m_lineBuffer.length() > 1)
	{
		...
		if (error)
		{
            ...
            */
            ...
            return new IPFSFile(fname, size, dir); // note such path can't be used to do our "magic" stream-in-strea-in-stream, you can use it only to list dir
            //return new IPFSFile("ml://c64.meatloaf.cc/dummy/file.prg", 123, false);
        }
...
bool IPFSIStream::open() {
    ...
    std::string ml_url = "https://ipfs.io/ipfs/" + urlParser.host;
