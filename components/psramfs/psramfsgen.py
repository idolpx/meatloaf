#!/usr/bin/env python
#
# psramfsgen is a tool used to generate a psram image from a directory
#
...
PSRAMFS_PH_FLAG_USED_FINAL_INDEX = 0xF8
PSRAMFS_PH_FLAG_USED_FINAL = 0xFC

PSRAMFS_PH_FLAG_LEN = 1
PSRAMFS_PH_IX_SIZE_LEN = 4
PSRAMFS_PH_IX_OBJ_TYPE_LEN = 1
PSRAMFS_TYPE_FILE = 1

# Based on typedefs under psramfs_config.h
PSRAMFS_OBJ_ID_LEN = 2  # psramfs_obj_id
PSRAMFS_SPAN_IX_LEN = 2  # psramfs_span_ix
PSRAMFS_PAGE_IX_LEN = 2  # psramfs_page_ix
PSRAMFS_BLOCK_IX_LEN = 2  # psramfs_block_ix
...
class SpiffsBuildConfig(object):
    def __init__(self,
        ...
        self.OBJ_DATA_PAGE_HEADER_LEN = self.obj_id_len + self.span_ix_len + PSRAMFS_PH_FLAG_LEN
        ...
        self.OBJ_INDEX_PAGES_HEADER_LEN = (self.OBJ_DATA_PAGE_HEADER_LEN_ALIGNED + PSRAMFS_PH_IX_SIZE_LEN +
                                           PSRAMFS_PH_IX_OBJ_TYPE_LEN + self.obj_name_len + self.meta_len)
        if aligned_obj_ix_tables:
            self.OBJ_INDEX_PAGES_HEADER_LEN_ALIGNED = (self.OBJ_INDEX_PAGES_HEADER_LEN + PSRAMFS_PAGE_IX_LEN - 1) & ~(PSRAMFS_PAGE_IX_LEN - 1)
...
class SpiffsObjLuPage(SpiffsPage):
    ...
    def _calc_magic(self, blocks_lim):  # type: (int) -> int
        # Calculate the magic value mirroring computation done by the macro PSRAMFS_MAGIC defined in
        # psramfs_nucleus.h
...
class SpiffsObjIndexPage(SpiffsObjPageWithIdx):
    ...
    def to_binary(self):  # type: () -> bytes
        ...
        img = struct.pack(SpiffsPage._endianness_dict[self.build_config.endianness] +
                          ...
                          SpiffsPage._len_dict[PSRAMFS_PH_FLAG_LEN],
                          obj_id,
                          self.span_ix,
                          PSRAMFS_PH_FLAG_USED_FINAL_INDEX)
        ...
        if self.span_ix == 0:
            img += struct.pack(SpiffsPage._endianness_dict[self.build_config.endianness] +
                               SpiffsPage._len_dict[PSRAMFS_PH_IX_SIZE_LEN]  +
                               SpiffsPage._len_dict[PSRAMFS_PH_FLAG_LEN],
                               self.size,
                               PSRAMFS_TYPE_FILE)
...
def main():  # type: () -> None
    parser = argparse.ArgumentParser(description='PSRAMFS Image Generator',
    ...
    parser.add_argument('--page-size',
                        help='Logical page size. Set to value same as CONFIG_PSRAMFS_PAGE_SIZE.',
                        type=int,
                        default=256)
    ...
    parser.add_argument('--obj-name-len',
                        help='File full path maximum length. Set to value same as CONFIG_PSRAMFS_OBJ_NAME_LEN.',
                        type=int,
                        default=32)

    parser.add_argument('--meta-len',
                        help='File metadata length. Set to value same as CONFIG_PSRAMFS_META_LENGTH.',
                        type=int,
                        default=4)

    parser.add_argument('--use-magic',
                        dest='use_magic',
                        help='Use magic number to create an identifiable PSRAMFS image. Specify if CONFIG_PSRAMFS_USE_MAGIC.',
                        action='store_true')
    ...
    parser.add_argument('--use-magic-len',
                        dest='use_magic_len',
                        help='Use position in memory to create different magic numbers for each block. Specify if CONFIG_PSRAMFS_USE_MAGIC_LENGTH.',
                        action='store_true')
    ...
    parser.add_argument('--aligned-obj-ix-tables',
                        action='store_true',
                        help='Use aligned object index tables. Specify if PSRAMFS_ALIGNED_OBJECT_INDEX_TABLES is set.')
    ...
    with open(args.output_file, 'wb') as image_file:
        ...
        psramfs_build_default = SpiffsBuildConfig(args.page_size, PSRAMFS_PAGE_IX_LEN,
                                                 args.block_size, PSRAMFS_BLOCK_IX_LEN, args.meta_len,
                                                 args.obj_name_len, PSRAMFS_OBJ_ID_LEN, PSRAMFS_SPAN_IX_LEN,
                                                 True, True, 'big' if args.big_endian else 'little',
                                                 args.use_magic, args.use_magic_len, args.aligned_obj_ix_tables)

        psram = SpiffsFS(image_size, psramfs_build_default)

        for root, dirs, files in os.walk(args.base_dir, followlinks=args.follow_symlinks):
            for f in files:
                full_path = os.path.join(root, f)
                psram.create_file('/' + os.path.relpath(full_path, args.base_dir).replace('\\', '/'), full_path)

        image = psram.to_binary()
