...
#define THISLOADER	RAINBOW_ISLANDS
...
#define HEADERSIZE	39	/* size of block header */
...
void rainbowislands_search (void)
{
	...
	for (i = 20; i > 0 && i < tap.len - BITSINABYTE; i++) {
		...
		if (eop > 0) {
			...
			/* Additional plausibility checks: padding and EOH */
			for (h = 6; h < HEADERSIZE - 1; h++)
				if (hd[h] != 0xFF)
					break;
			if (h != HEADERSIZE - 1)
				continue;
...
int rainbowislands_describe(int row)
{
	int i, s;
	int hd[HEADERSIZE];
	int en, tp, sp, lp;
