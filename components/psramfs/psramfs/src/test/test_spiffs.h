...
#ifndef TEST_PSRAMFS_H_
#define TEST_PSRAMFS_H_
...
#define FS_PURE_DATA_PAGES(fs) \
    (PSRAMFS_CFG_PHYS_SZ(fs) / PSRAMFS_CFG_LOG_PAGE_SZ(fs)- (fs)->block_count * PSRAMFS_OBJ_LOOKUP_PAGES(fs))
#define FS_PURE_DATA_SIZE(fs) \
    FS_PURE_DATA_PAGES(fs) * PSRAMFS_DATA_PAGE_SIZE(fs)
