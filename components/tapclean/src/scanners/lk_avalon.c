/*
 * lk_avalon.c (by Luigi Di Fraia, March 2021)
 * Takes its name from the Polish publisher "LK Avalon"
 * Based on tequila.c
 *
...
#define ENDOFFSETH	0	/* end  location (MSB) offset inside header */
#define ENDOFFSETL	1	/* end  location (LSB) offset inside header */
...
int lk_avalon_describe(int row)
{
	int i, s;
	int hd[HEADERSIZE];
	int en, tp, sp, lp;
