...
- **Observed, not fixed**: on that run the gzip ISIZE probe read `1835886970` (`0x6D6F632E`, the ASCII bytes `.com`) instead of the trailer, i.e. the seek-to-EOF served bytes from somewhere else entirely. The 8 MB sanity cap caught it and the byte-count fallback produced the right size, so extraction was correct — but it cost two extra full decompression passes over HTTP, and it means an EOF-relative read on a network stream can still return the wrong bytes.
