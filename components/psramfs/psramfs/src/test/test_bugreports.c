...
TEST(nodemcu_full_fs_1) {
  ...
  fd = PSRAMFS_open(FS, "test1.txt", PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
  TEST_CHECK(fd > 0);
  int i;
  psramfs_stat s;
  res = PSRAMFS_OK;
  for (i = 0; i < 100*1000; i++) {
    u8_t buf = 'x';
    res = PSRAMFS_write(FS, fd, &buf, 1);
  }

  int errno = PSRAMFS_errno(FS);
  int res2 = PSRAMFS_fstat(FS, fd, &s);
  TEST_CHECK(res2 == PSRAMFS_OK);
  printf("  >>> file %s size: %i\n", s.name, s.size);

  TEST_CHECK(errno == PSRAMFS_ERR_FULL);
  PSRAMFS_close(FS, fd);

  printf("  remove big file\n");
  res = PSRAMFS_remove(FS, "test1.txt");

  printf("res:%i errno:%i\n",res, PSRAMFS_errno(FS));

  TEST_CHECK(res == PSRAMFS_OK);
  res2 = PSRAMFS_fstat(FS, fd, &s);
  TEST_CHECK(res2 < 0);
  TEST_CHECK(PSRAMFS_errno(FS) == PSRAMFS_ERR_FILE_CLOSED);
  res2 = PSRAMFS_stat(FS, "test1.txt", &s);
  TEST_CHECK(res2 < 0);
  TEST_CHECK(PSRAMFS_errno(FS) == PSRAMFS_ERR_NOT_FOUND);

  printf("  create small file\n");
  fd = PSRAMFS_open(FS, "test2.txt", PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
  TEST_CHECK(fd > 0);
  res = PSRAMFS_OK;
...
TEST(nodemcu_full_fs_2) {
  ...
  TEST_CHECK(PSRAMFS_errno(FS) == PSRAMFS_ERR_FULL);
  res2 = PSRAMFS_fstat(FS, fd, &s);
  TEST_CHECK(res2 == PSRAMFS_OK);
  printf("  >>> file %s size: %i\n", s.name, s.size);
  TEST_CHECK(s.size == 0);
  ...
  TEST_CHECK(res == PSRAMFS_OK);
#if 0
  res = PSRAMFS_remove(FS, "test2.txt");
  TEST_CHECK(res == PSRAMFS_OK);
#endif

  printf("  create medium file\n");
  fd = PSRAMFS_open(FS, "test3.txt", PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
  TEST_CHECK(PSRAMFS_errno(FS) == PSRAMFS_OK);
  TEST_CHECK(fd > 0);
...
TEST(magic_test) {
  ...
  TEST_ASSERT(sizeof(psramfs_obj_id) == sizeof(u16_t));

  // one obj lu page, not full
  fs_reset_specific(0, 0, 4096*16, 4096, 4096*1, 128);
  TEST_CHECK(PSRAMFS_CHECK_MAGIC_POSSIBLE(FS));
...
TEST(nodemcu_309) {
  ...
  for (j = 1; j <= 3; j++) {
    ...
    fd = PSRAMFS_open(FS, fname, PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC | PSRAMFS_DIRECT, 0);
    TEST_CHECK(fd > 0);
    int i;
    res = PSRAMFS_OK;
    u8_t err = 0;
    for (i = 1; i <= 1280; i++) {
      ...
      res = PSRAMFS_write(FS, fd, buf, strlen(buf));
      if (!err && res < 0) {
        printf("err @ %i,%i\n", i, j);
        err = 1;
      }
...
TEST(robert) {
  ...
  fd = PSRAMFS_open(FS, fname, PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
  TEST_CHECK(fd > 0);
  res = PSRAMFS_OK;
  char buf[500];
  memset(buf, 0xaa, 500);
  res = PSRAMFS_write(FS, fd, buf, 500);
  TEST_CHECK(res >= PSRAMFS_OK);
  PSRAMFS_close(FS, fd);

  int errno = PSRAMFS_errno(FS);
  TEST_CHECK(errno == PSRAMFS_OK);
  ...
  psramfs_stat s;
  TEST_CHECK(PSRAMFS_stat(FS, fname, &s) == PSRAMFS_OK);
...
TEST(zero_sized_file_44) {
  ...
  psramfs_file fd = PSRAMFS_open(FS, "zero", PSRAMFS_RDWR | PSRAMFS_CREAT, 0);
  TEST_CHECK_GE(fd, 0);

  int res = PSRAMFS_close(FS, fd);
  TEST_CHECK_GE(res, 0);

  fd = PSRAMFS_open(FS, "zero", PSRAMFS_RDWR, 0);
  TEST_CHECK_GE(fd, 0);

  u8_t buf[8];
  res = PSRAMFS_read(FS, fd, buf, 8);
  TEST_CHECK_EQ(res, 0);
  TEST_CHECK_EQ(PSRAMFS_errno(FS), PSRAMFS_ERR_END_OF_OBJECT);

  res = PSRAMFS_read(FS, fd, buf, 0);
  TEST_CHECK_GE(res, 0);
...
TEST(truncate_48) {
  ...
  u32_t len = PSRAMFS_DATA_PAGE_SIZE(FS)/2;

  s32_t res = test_create_and_write_file("small", len, len);
  TEST_CHECK_GE(res, 0);

  psramfs_file fd = PSRAMFS_open(FS, "small", PSRAMFS_RDWR, 0);
  TEST_CHECK_GE(fd, 0);

  psramfs_fd *desc;
#if PSRAMFS_FILEHDL_OFFSET
  res = psramfs_fd_get(FS, fd - TEST_PSRAMFS_FILEHDL_OFFSET, &desc);
#else
  res = psramfs_fd_get(FS, fd, &desc);
#endif
  ...
  u32_t new_len = len/2;
  res = psramfs_object_truncate(desc, new_len, 0);
  TEST_CHECK_GE(res, 0);
  ...
  psramfs_stat s;
  res = PSRAMFS_stat(FS, "small", &s);
  TEST_CHECK_GE(res, 0);
  TEST_CHECK_EQ(s.size, new_len);

  res = PSRAMFS_remove(FS, "small");
  TEST_CHECK_GE(res, 0);

  fd = PSRAMFS_open(FS, "small", PSRAMFS_RDWR, 0);
  TEST_CHECK_LT(fd, 0);
  TEST_CHECK_EQ(PSRAMFS_errno(FS), PSRAMFS_ERR_NOT_FOUND);
...
TEST(eof_tell_72) {
  ...
  psramfs_file fd = PSRAMFS_open(FS, "file", PSRAMFS_CREAT | PSRAMFS_RDWR | PSRAMFS_APPEND, 0);
  TEST_CHECK_GT(fd, 0);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 1);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 0);

  res = PSRAMFS_write(FS, fd, "test", 4);
  TEST_CHECK_EQ(res, 4);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 1);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 4);

  res = PSRAMFS_fflush(FS, fd);
  TEST_CHECK_EQ(res, PSRAMFS_OK);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 1);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 4);

  res = PSRAMFS_lseek(FS, fd, 2, PSRAMFS_SEEK_SET);
  TEST_CHECK_EQ(res, 2);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 0);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 2);

  res = PSRAMFS_write(FS, fd, "test", 4);
  TEST_CHECK_EQ(res, 4);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 1);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 8);
  ...
  res = PSRAMFS_close(FS, fd);
  TEST_CHECK_EQ(res, PSRAMFS_OK);
  TEST_CHECK_LT(PSRAMFS_eof(FS, fd), PSRAMFS_OK);
  TEST_CHECK_LT(PSRAMFS_tell(FS, fd), PSRAMFS_OK);

  fd = PSRAMFS_open(FS, "file", PSRAMFS_RDWR, 0);
  TEST_CHECK_GT(fd, 0);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 0);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 0);
  ...
  TEST_CHECK_EQ(res, 4);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 0);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 6);
  ...
  res = PSRAMFS_lseek(FS, fd, 0, PSRAMFS_SEEK_END);
  TEST_CHECK_EQ(res, 8);
  TEST_CHECK_EQ(PSRAMFS_eof(FS, fd), 1);
  TEST_CHECK_EQ(PSRAMFS_tell(FS, fd), 8);
...
TEST(psramfs_dup_file_74) {
  fs_reset_specific(0, 0, 64*1024, 4096, 4096*2, 256);
  {
    psramfs_file fd = PSRAMFS_open(FS, "/config", PSRAMFS_CREAT | PSRAMFS_TRUNC | PSRAMFS_WRONLY, 0);
    TEST_CHECK(fd >= 0);
    char buf[5];
    strncpy(buf, "test", sizeof(buf));
    PSRAMFS_write(FS, fd, buf, 4);
    PSRAMFS_close(FS, fd);
  }
  {
    psramfs_file fd = PSRAMFS_open(FS, "/data", PSRAMFS_CREAT | PSRAMFS_TRUNC | PSRAMFS_WRONLY, 0);
    TEST_CHECK(fd >= 0);
    PSRAMFS_close(FS, fd);
  }
  {
    psramfs_file fd = PSRAMFS_open(FS, "/config", PSRAMFS_RDONLY, 0);
    TEST_CHECK(fd >= 0);
    char buf[5];
    int cb = PSRAMFS_read(FS, fd, buf, sizeof(buf));
  ...
  }
  {
    ...
    TEST_CHECK(fd >= 0);
    psramfs_stat stat;
    PSRAMFS_fstat(FS, fd, &stat);
    if (strcmp((const char*) stat.name, "/data") != 0) {
      // oops! lets check the list of files...
      psramfs_DIR dir;
      PSRAMFS_opendir(FS, "/", &dir);
      struct psramfs_dirent dirent;
      while (PSRAMFS_readdir(&dir, &dirent)) {
        printf("%s\n", dirent.name);
      }
...
TEST(temporal_fd_cache) {
  ...
  TEST_CHECK_EQ(test_create_and_write_file(fcss, 2000, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[0], 4000, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[1], 3000, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[2], 2000, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[3], 1000, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[4], 1500, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[5], 3000, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[6], 2000, 256), PSRAMFS_OK);
  TEST_CHECK_EQ(test_create_and_write_file(fhtml[7], 3500, 256), PSRAMFS_OK);
  ...
  do {
    ...
    TEST_CHECK_EQ(read_and_verify(fhtml[html_ix]), PSRAMFS_OK);

    // open & read css
    TEST_CHECK_EQ(read_and_verify(fcss), PSRAMFS_OK);
  } while (run ++ < runs);
...
static int run_fuzz_test(FILE *f, int maxfds, int debuglog) {
  ...
  // The list of 8 modes that are chosen. PSRAMFS_EXCL is not present -- it probably ought to be.
  int modes[8] = {PSRAMFS_RDONLY, PSRAMFS_RDWR, PSRAMFS_RDWR|PSRAMFS_TRUNC, PSRAMFS_RDWR|PSRAMFS_CREAT, PSRAMFS_RDWR|PSRAMFS_CREAT|PSRAMFS_TRUNC,
      PSRAMFS_WRONLY|PSRAMFS_CREAT|PSRAMFS_TRUNC, PSRAMFS_RDWR|PSRAMFS_CREAT|PSRAMFS_TRUNC|PSRAMFS_DIRECT, PSRAMFS_WRONLY};
...
#define LOGOP if (debuglog) printf
  ...
  while ((c = fgetc(f)) >= 0) {
    ...
    case 'O':
      ...
      fd[fdn] = PSRAMFS_open(FS, filename[(arg>>3) & 7], modes[arg & 7], 0);
      if (fd[fdn] >= 0) {
    ...
    case 'S':
      if (fd[fdn] >= 0) {
        ...
        PSRAMFS_lseek(FS, fd[fdn], offset, whence);
      }
      break;
    ...
    case 'R':
      if (fd[fdn] >= 0) {
        ...
        int rlen = PSRAMFS_read(FS, fd[fdn], rbuff, (15 << (arg & 7)) + (arg & 127));
        LOGOP(" -> %d\n", rlen);
      }
      break;
    ...
    case 'W':
      if (fd[fdn] >= 0) {
        ...
        rc = PSRAMFS_write(FS, fd[fdn], buff, (15 << (arg & 7)) + (arg & 127));
        LOGOP(" -> %d\n", rc);
      }
      break;
    ...
    rc = PSRAMFS_remove(FS, filename[arg & 7]);
    LOGOP(" -> %d\n", rc);
    break;
    ...
    rc = PSRAMFS_rename(FS, filename[arg & 7], filename[(arg >> 3) & 7]);
    LOGOP(" -> %d\n", rc);
    break;
...
TEST(small_free_space) {
  ...
  int tfd = PSRAMFS_open(FS, "testfile", PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
  TEST_CHECK(tfd > 0);
  char *tbuf = "some data";
  res = PSRAMFS_write(FS, tfd, tbuf, strlen(tbuf));

  TEST_CHECK(res == strlen(tbuf));

  res = PSRAMFS_fflush(FS, tfd);
  TEST_CHECK(res >= PSRAMFS_OK);

  PSRAMFS_close(FS, tfd);
  ...
  do {
    ...
    fd = PSRAMFS_open(FS, fileCurrName, PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
    TEST_CHECK(fd > 0);
    ...
    if (fileCntr > maxFileNr) {
      ...
      res = PSRAMFS_remove(FS, fileDelName);
  ...
  tfd = PSRAMFS_open(FS, "testfile", PSRAMFS_RDONLY, 0);
  TEST_CHECK(tfd > 0);
  char rbuf[32];
  res = PSRAMFS_read(FS, tfd, rbuf, sizeof(rbuf));
...
TEST(lots_of_overwrite) {
  ...
  for (i = 0; i < 5; i++) {
    ...
    int tfd = PSRAMFS_open(FS, filename, PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
    TEST_CHECK(tfd > 0);
    char tbuf[1024];
    memset(tbuf, 'a', 700);
    tbuf[700] = 0;
  ...
  for (run = 0; run < runs; run++) {
    ...
    int tfd = PSRAMFS_open(FS, "file.dat", PSRAMFS_RDWR | PSRAMFS_CREAT | PSRAMFS_TRUNC, 0);
    TEST_CHECK(tfd > 0);
    res = PSRAMFS_write(FS, tfd, buf, strlen(buf));
    ...
    tfd = PSRAMFS_open(FS, "file.dat", PSRAMFS_RDONLY, 0);
    TEST_CHECK(tfd > 0);
    char rbuf[2000];
    res = PSRAMFS_read(FS, tfd, rbuf, sizeof(rbuf));
    ...
    tfd = PSRAMFS_open(FS, filename, PSRAMFS_RDONLY, 0);
    TEST_CHECK(tfd > 0);
    char tbuf[1024];
    memset(tbuf, 'a', 700);
    tbuf[700] = 0;
...
#if 0
TEST(psramfs_hidden_file_90) {
  fs_mount_dump("imgs/90.hidden_file.psram", 0, 0, 1*1024*1024, 4096, 4096, 128);
...
TEST(null_deref_check_93) {
  ...
  //int res = PSRAMFS_open(FS, "d43.fw", PSRAMFS_TRUNC | PSRAMFS_CREAT | PSRAMFS_WRONLY, 0);
...
TEST(psramfs_145) {
  ...
  {
    psramfs_file fd = PSRAMFS_open(FS, "biggie", PSRAMFS_CREAT | PSRAMFS_TRUNC | PSRAMFS_WRONLY, 0);
    TEST_CHECK(fd >= 0);
    char buf[1024*512];
    memset(buf, 0xee, sizeof(buf));
    TEST_CHECK_GT(PSRAMFS_write(FS, fd, buf, sizeof(buf)), 0);
    TEST_CHECK_EQ(PSRAMFS_close(FS, fd), PSRAMFS_OK);
  }
  const int runs = 1000;
  int run = 0;
  while (run++ < runs) {
    psramfs_file fd = PSRAMFS_open(FS, "clobber", PSRAMFS_CREAT | PSRAMFS_TRUNC | PSRAMFS_WRONLY, 0);
    TEST_CHECK(fd >= 0);
    char buf[8192];
    memset(buf, 0xee, sizeof(buf));
    TEST_CHECK_GT(PSRAMFS_write(FS, fd, buf, sizeof(buf)), 0);
    TEST_CHECK_EQ(PSRAMFS_close(FS, fd), PSRAMFS_OK);
    TEST_CHECK_EQ(PSRAMFS_remove(FS, "clobber"), PSRAMFS_OK);
  }

  // below stolen from PSRAMFS_vis
  psram *fs = FS;
  ...
  while (bix < fs->block_count) {
    ...
    TEST_CHECK_EQ(_psramfs_rd(fs, PSRAMFS_OP_C_READ | PSRAMFS_OP_T_OBJ_LU2, 0,
        PSRAMFS_ERASE_COUNT_PADDR(fs, bix),
        sizeof(psramfs_obj_id), (u8_t *)&erase_count), PSRAMFS_OK);
    TEST_CHECK_NEQ(erase_count, (psramfs_obj_id)-1);
...
TEST(seek_bug_148) {
  int res;
#define MAGIC_SIZE_THAT_FAILS 26355 // happens to be PSRAMFS_DATA_PAGE_SIZE(FS) * PSRAMFS_OBJ_HDR_IX_LEN(FS)
  fs_reset_specific(0, 0, 64*1024, 4096, 4096, 256);
  u8_t buf[MAGIC_SIZE_THAT_FAILS];
  psramfs_file fd = PSRAMFS_open(FS, "EVENT", PSRAMFS_O_CREAT | PSRAMFS_O_RDWR, 0);
  TEST_CHECK_GT(fd, 0);
  TEST_CHECK_EQ(PSRAMFS_write(FS, fd, &buf, sizeof(buf)), sizeof(buf));
  TEST_CHECK_EQ(PSRAMFS_close(FS, fd), PSRAMFS_OK);
  fd = PSRAMFS_open(FS, "EVENT", PSRAMFS_O_RDONLY, 0);
  TEST_CHECK_GT(fd, 0);
  TEST_CHECK_EQ(PSRAMFS_lseek(FS, fd, 0, SEEK_END), MAGIC_SIZE_THAT_FAILS);
  return TEST_RES_OK;
} TEST_END
...
TEST(remove_release_fd_152) {
  ...
  psramfs_file fd1 = PSRAMFS_open(FS, "removemeandloseafd", PSRAMFS_O_CREAT | PSRAMFS_O_RDWR, 0);
  TEST_CHECK_GT(fd1, 0);
  TEST_CHECK_EQ(count_taken_fds(FS), 1);
  TEST_CHECK_EQ(PSRAMFS_write(FS, fd1, &buf, sizeof(buf)), sizeof(buf));
  TEST_CHECK_EQ(PSRAMFS_close(FS, fd1), PSRAMFS_OK);
  TEST_CHECK_EQ(count_taken_fds(FS), 0);
  psramfs_file fd2 = PSRAMFS_open(FS, "removemeandloseafd", PSRAMFS_O_RDWR, 0);
  TEST_CHECK_GT(fd2, 0);
  TEST_CHECK_EQ(count_taken_fds(FS), 1);
  psramfs_file fd3 = PSRAMFS_open(FS, "removemeandloseafd", PSRAMFS_O_RDWR, 0);
  TEST_CHECK_GT(fd3, 0);
  TEST_CHECK_EQ(count_taken_fds(FS), 2);
  TEST_CHECK_EQ(PSRAMFS_remove(FS, "removemeandloseafd"), PSRAMFS_OK);
  TEST_CHECK_EQ(count_taken_fds(FS), 0);
  return TEST_RES_OK;
} TEST_END
...
TEST(certain_file_size_fail_165) {
  ...
  TEST_CHECK_EQ(PSRAMFS_creat(FS, "test", 0), PSRAMFS_OK);
  psramfs_file fd = PSRAMFS_open(FS, "test", PSRAMFS_O_CREAT | PSRAMFS_O_WRONLY, 0);
  TEST_CHECK_GT(fd, 0);

  int i;
  for (i = 0; i < NUM; i++) {
    TEST_CHECK_EQ(PSRAMFS_write(FS, fd, buf, SIZ), SIZ);
  }
  TEST_CHECK_EQ(PSRAMFS_close(FS, fd), PSRAMFS_OK);
  fd = PSRAMFS_open(FS, "test", PSRAMFS_O_RDONLY, 0);
  TEST_CHECK_GT(fd, 0);

  psramfs_stat s;
  TEST_CHECK_EQ(PSRAMFS_fstat(FS, fd, &s), PSRAMFS_OK);
  TEST_CHECK_EQ(s.size, NUM*SIZ);

  int size = 0;
  for (i = 0; i < NUM; i++) {
    size += PSRAMFS_read(FS, fd, buf, SIZ);
  }
...
SUITE_TESTS(bug_tests)
  ...
  ADD_TEST(psramfs_dup_file_74)
  ADD_TEST(temporal_fd_cache)
  ADD_TEST(psramfs_145)
...
#if 0
  ADD_TEST(psramfs_hidden_file_90)
#endif
#if 0
