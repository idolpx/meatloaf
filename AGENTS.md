...
- **Not fixed, worth knowing**: that file's gzip header sets FNAME, carrying the original name (`ordeal +2 100% (ntsc …`), which is richer than anything the URL can give. The `isRawCompressedEntry` branch already prefers the embedded name when libarchive exposes one, but the compressed-only branch (`archive_read_next_header` → `ARCHIVE_EOF`) never sees an entry and so always falls back to the URL.
