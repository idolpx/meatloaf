...
void
shift_buffer(uint8_t * buffer, int length, int n)
{
	int i;
	uint8_t tempbuf[NIB_TRACK_LENGTH];
	uint8_t carry;
	int carryshift;
...
uint8_t *
align_vmax(uint8_t * work_buffer, int tracklen)
{
	uint8_t *pos, *buffer_end, *start_pos;
	int run;
...
uint8_t *
auto_gap(uint8_t * work_buffer, int tracklen)
{
	uint8_t *pos, *buffer_end, *key_temp, *key;
	int run, longest;
...
uint8_t *
find_weak_gap(uint8_t * work_buffer, int tracklen)
{
	uint8_t *pos, *buffer_end, *key_temp, *key;
	int run, longest;
...
// this helps some custom protection tracks master properly
uint8_t *
find_long_sync(uint8_t * work_buffer, int tracklen)
{
	uint8_t *pos, *buffer_end, *key_temp, *key;
	int run, longest;
