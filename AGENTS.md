...
- **Not covered**: writing INTO a disk image through a channel. `cp` refuses to copy a `.d64` (it
  resolves as a directory), so no scratch image could be made without modifying a real one. The
  console plumbing is identical either way — a plain-SD write exercises the same
  console → channel → stream path — and the in-image engine has its own 105-case suite.
