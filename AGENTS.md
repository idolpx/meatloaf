...
- **The resolved entry name now reaches the extracted file** (`media/archive/archive.h`, `lib/console/Commands/VFSCommands.cpp`): with esp-idf#18359 patched, a `.gz` over HTTPS extracted the full 174848 bytes but landed as `ordeal%2b2100p.d64` — the archive layer had resolved `ordeal +2 100% (ntsc pal) wanderer.d64` from the gzip FNAME, and `unzipx` was still naming the output from `srcFile->name`, the raw URL basename. `ArchiveMFile::getDecodedStream()` records what the entry resolved to (`m_resolvedName`) and `getDownloadFilename()` returns it — the same hook `wget` uses for `Content-Disposition`. It must be asked AFTER the stream is opened: resolving the entry is what discovers the name, and `getDecodedStream()` then calls `resetURL(base())`, which empties `name`. **Rule: an MFile's identity does not survive `getSourceStream()`** — capture it first, or ask `getDownloadFilename()` afterwards.
  Covered by `test_download_filename_is_the_resolved_entry`, which needed `MFile`'s one-argument constructor on the host; `meatloaf.cpp` cannot compile natively, so `host_stubs.cpp` carries its real one-line body (`resetURL(path)`) verbatim, flagged to be kept in sync.
- **The real fix is in ESP-IDF, and is applied by a build script** — **HARDWARE-VERIFIED 2026-08-12: `unzipx` of a `.gz` over HTTPS works.** (`patch_framework.py`, new; wired into `[esp32_base]` as `pre:`): **esp-idf PR #18359 / IDFGH-17389** — `esp_http_client_prepare()` reset `data_process` and `first_line_prepared` but never reset the RESPONSE BUFFER, so `raw_data` stayed wherever the previous response stopped and a new request on the same handle parsed the previous response's leftovers as its own. That single defect explains every archive symptom over HTTPS in this session: buffers handed back unwritten (`0xBAAD5678` canaries, `.com`/`core` ASCII where a gzip trailer belongs), a gzip filter lost on re-open (a 174848-byte D64 measured as its 52223-byte compressed length), and a NULL-pointer `memcpy` inside `esp_http_client_read()`. The patch adds `esp_http_client_cached_buf_cleanup(client->response->buffer)` + `raw_len = 0` to `esp_http_client_prepare()`.
  **Why a script rather than a hand-edit**: the framework lives in `~/.platformio/packages/`, outside this repo — an edit there is invisible to git, lost on package reinstall/update, and absent for every other checkout. `patch_framework.py` is idempotent (marker `MEATLOAF-PATCH esp-idf#18359`), refuses to patch if the file does not match the expected shape, and should be DROPPED once the framework package ships the fix. Note that touching a framework file makes PlatformIO invalidate every environment's build tree; on Windows that cleanup can fail half-way and leave a state where CMake reports "the C compiler is not able to compile a simple test program" — `rm -rf .pio/build` and rebuild. After that wipe the first build can also fail to link with `cannot find .../libtapclean.a` — the archive is still being written when ld runs; simply build again.
  **`canProbeGzipTrailer()` has been removed again** now that the cause is fixed: the gzip ISIZE trailer probe runs for network sources as before, so a `.gz` over HTTP has a known size without decompressing it (with the guard in place a directory listing showed 0). The seek was never the fault. Two guards from the hunt are KEPT deliberately, both cheap and both turning a recurrence into a loud failure rather than silent corruption: the handle rebuild in `MeatHttpClient::seek()` when a response cannot be drained (costs nothing unless a drain fails), and the check in `seekEntry()` that refuses to measure a stream which lost its compression filter (what stopped a 174848-byte D64 being written as 52223 bytes).
- **Root cause of the HTTP archive corruption, finally named** (`lib/meatloaf/network/http.cpp`): `esp_http_client_open()` on a handle whose previous GET response was **not fully consumed** does not regenerate the request line — ESP-IDF leaves `first_line_prepared` set for GET/HEAD, which `MeatHttpClient::init()`'s own comment already documents ("the request line is never regenerated and the server hangs. Recreate instead"). The client then sends a stale request and parses whatever comes back as the response it asked for. Every archive failure over HTTPS this session is a symptom of it: read buffers handed back **unwritten** (`0xBAAD5678` heap canaries, stray `.com`/`core` ASCII), a gzip filter **lost on re-open** so a 174848-byte D64 was measured as its 52223-byte compressed length, and finally a **NULL-pointer `memcpy` inside `esp_http_client_read()`** during filter bidding. `MeatHttpClient::seek()` drained the response with `flush(0)` and then re-opened regardless of whether the drain worked — `flush(0)` discarded `esp_http_client_flush_response()`'s return. It now returns that result, and seek REBUILDS the handle via `init()` when the drain did not complete. Costs a fresh connection on that path, which is the state the client demonstrably works in: the one time this recovered on hardware was when the re-request failed and the error path called `init()`. **Unverified — `MeatHttpClient` needs `esp_http_client` and cannot be compiled natively.**
- **Percent-encoding from a URL leaked into extracted filenames** (`lib/meatloaf/media/archive/archive.cpp`): `unzipx https://…/ordeal%2b2100p.d64.gz` wrote a file literally named `ordeal%2b2100p.d64`. A single compressed file carries no directory, so the archive layer synthesises the entry name from the container's own path — and a URL path component is percent-encoded. Both sites that did this (the compressed-only branch and the `isRawCompressedEntry` branch) now share `compressedEntryNameFromUrl()`, which decodes **only when the path contains `://`**: a LOCAL file genuinely named with a `%` must keep it. `alter_pluses` is false, because `+` in a path is a literal plus rather than the form-encoded space it means in a query string — the same call `fnFsHTTP.cpp` makes for HTTP directory listings. Covered both ways by `test_archive_extract`.
- **The gzip FNAME header is now preferred over the URL** (`archive.cpp`, `Archive::gzipNameFromHeader()`): a `.gz` stores the ORIGINAL filename, which the URL cannot reproduce — zimmers' `ordeal%2b2100p.d64.gz` is `ordeal +2 100% (ntsc pal) wanderer.d64` inside. libarchive exposes FNAME as the entry pathname, but ONLY when a format reader yields an entry; a compressed-only file returns `ARCHIVE_EOF`, so that branch never saw it. It is now parsed straight from the header (RFC 1952: 10-byte fixed part, optional FEXTRA, then a NUL-terminated name) out of the leading bytes **`cb_read` already captured** — no extra read and no extra seek, which matters because seeking is exactly what corrupts these streams. A name that would run past the captured bytes is rejected rather than truncated, and any directory part is stripped. Falls back to the URL-derived name. `m_firstBytes` grew 16 → 256 bytes for this; the failure diagnostic still prints only the first 16.
- **`.gz` over HTTPS is STILL extracted truncated, and the numbers prove it**: `.archive/gz/ordeal+2100p.d64.gz` is 52223 compressed and **174848 decompressed**, its ISIZE field says 174848 — and the device read that field as `1702132066` (`0x65726F63`, ASCII `core`) and then extracted 52223 bytes, the compressed length. So both the EOF-relative ISIZE read AND the `rawOnly` re-open that follows are served something other than the file, and the "single-pass" `loadUnknownSize()` measured the undecompressed stream. The ISIZE probe's seek to EOF-4 and back is the trigger. **Fixed by not probing a network source at all** (`canProbeGzipTrailer()`, gating both probe sites): the probe is only an optimisation — it learns the size without decompressing — and `ensureData()` already falls back to a single pass that determines the size WHILE extracting, needing no seek. Locally the seek is free and reliable so the optimisation is kept, which is what `test_trailer_probe_still_used_for_a_local_file` pins. **Cost: a `.gz` served over HTTP has no known size until its content is read, so a directory listing shows 0 for one.** The underlying HTTP fault is still not fixed — this removes the thing that triggers it.

- **`unzipx` of a single compressed file wrote to the destination DIRECTORY, not a file in it** (`lib/console/Commands/VFSCommands.cpp`): `unzipx https://…/Elite.d64.gz` printed `/sd/.bin/  (41448 bytes)` and failed with `fopen … errno[13:Permission denied]`. The decompression was entirely correct — the entry name was lost. **`MFile::getSourceStream()` MUTATES the MFile for a single-file compression**: `ArchiveMFile::getDecodedStream()` ends with `resetURL(base())`, repointing it at the containing directory so the CWD is right after a `LOAD`, which leaves `name` empty. `unzipx` read `srcFile->name` AFTER that call, so the path collapsed to `<dest>/`. Fixed by taking the name first. Same shape as the earlier `wget` filename bug: **anything that needs an MFile's identity across a `getSourceStream()` call must capture it beforehand.** Not covered by the native suite — `lib/console` is not compiled there.
- **That same run also extracted a TRUNCATED file, and the log said so** (`lib/meatloaf/media/archive/archive.cpp`, `seekEntry(uint16_t)`): `gzip ISIZE check: srcSize=41448` (the COMPRESSED length) was followed by `Counted decompressed size: 41448` for a `.gz` whose real content is a 174848-byte D64 — the two are only equal because the byte-count fallback measured the wrong stream. Sequence: the ISIZE probe read `0x6D6F632E` (ASCII `.com`) instead of the gzip trailer, so the 8 MB sanity cap correctly rejected it; the re-open before the fallback then came back **`filter count: 1 / none`** — no gzip filter — so `archive_read_data()` returned the raw compressed bytes and counted 41448; the caller extracted exactly that many decompressed bytes. **The byte-count fallback now refuses to measure a stream that lost its compression filter** — gated on whether the archive HAD a compression filter before the probe re-opened it, which is not the same question as `isRawCompressedEntry`: a gzip stream carrying an FNAME header (`Elite.d64.gz` does) yields a real pathname, so that flag is false and only the `entry.size == 0` arm reaches the size determination. Gating on it meant the first version of this guard never fired: it re-opens once more (the hardware trace shows the NEXT open did come back with the filter) and fails rather than reporting the compressed length as the entry size. The upstream cause is the same one behind every other bug in this entry — the HTTP source serving something other than the file's start after a seek — and is NOT fixed by this; the guard only stops it surfacing as silent truncation. **Could not be reproduced natively** (several models of a desynchronising source failed to put the archive in the filter-less state), so `test_archive_extract`'s `test_gz_size_is_the_decompressed_length` passes both before and after — it guards the fallback's arithmetic, it is not evidence the guard works. Unverified on hardware.

- **`unzipx` over HTTPS failed on the first attempt and worked on the second** (`lib/meatloaf/network/http.cpp`): `MeatHttpClient::seek()`'s `isFriendlySkipper` path drained the live response and re-requested the same bytes even when the target equalled `_position`. Re-using the handle for a fresh request straight after flushing the previous one desynchronises it, and the reads that follow return the buffer **unwritten** — `first bytes[50 78 56 AD BA 56 AD BA AD BA BA 00 7A 69 70 00]` is ESP-IDF's `0xBAAD5678` heap tail canary plus a stray `"zip"` — while still reporting a positive byte count. `Archive::open()`'s `seek(0)` on a freshly opened stream is exactly that redundant seek, which is why the FIRST attempt always failed; the second only recovered because its re-request failed (`httpCode=-1`) and the failure path calls `init()`, tearing the handle down and rebuilding it clean. Now short-circuits on `_is_open && pos == _position && !complete()`. **Hardware-verified**: the first attempt after a fresh boot now opens the archive and extracts all 30 entries, where every prior first attempt failed.
- **Internal-DRAM exhaustion fixed by `CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=128`** (all 21 PSRAM board sdkconfigs + `sdkconfig.defaults`): free internal during `unzipx` went from `Low[1504]` to `Low[8740]`, the `sdmmc_read_sectors: not enough mem` storm stopped, the `abort()` in `fopen()` stopped, and a 30-entry / 910 KB archive extracted completely. The threshold is the cutoff below which an allocation is served from internal DRAM instead of PSRAM; at 512 it captured the FATFS per-file sector cache (512 B), the LFN working buffer (~512 B at `CONFIG_FATFS_MAX_LFN=255`), newlib `FILE`s and most `MFile`/`std::string` traffic — roughly 1.5 KB of internal DRAM per open file. **Hardware-verified on lolin-d32-pro only**; propagated to the rest at the maintainer's direction. `freenove-esp32-cam` was already 128 independently of this work, and `esp32-s3-rgb` was on the IDF default of 1024. The 7 non-PSRAM configs do not have the key and must not get it. Revert a board to 512 if it regresses; the tradeoff is PSRAM cache-miss latency on small allocations.
- **libarchive compiled `-Os` to restore the ESP32 flash-text margin** (`components/libarchive/config.h`): `#pragma GCC optimize("Os")` guarded by `ESP_PLATFORM && __GNUC__`, the third use of this pattern after `components/sqlite3/private_include/config_ext.h` and `components/tapclean/src/mydefs.h` — and for the same reason, that the PlatformIO ESP-IDF builder silently strips per-component `-O` compile options (build.ninja shows the flag, the compile does not use it). Every libarchive source includes `archive_platform.h`, which includes `config.h` first, so one pragma reaches all of them. libarchive was the largest component still at the project's global `-Og` (`CONFIG_COMPILER_OPTIMIZATION_DEBUG`) at 523 KB of text — `sqlite3` (547 KB) and `tapclean` (155 KB) were already `-Os`. **Recovered 63 KB of flash text** (523.1 → 460.0 KB) plus ~4 KB of static RAM, which took `fujiloaf-rev0` from 241 bytes over the `iram0_2_seg` limit to linking with real headroom (Flash 42.1%, RAM 31.2%). libarchive is decode- and I/O-bound so the speed trade is not measurable here. The `ESP_PLATFORM` guard keeps `test/native/test_archive_extract` building at the host's own optimization level.
- **Board build survey taken during that rollout** (2026-08-12). ESP32-family PSRAM boards — the ones with the small ~3.3 MB `iram0_2_seg` flash-text window — are the exposed set; the S3s have room (`esp32-s3-devkitc-1` links at Flash 81.7%). Results: `lolin-d32-pro` ✅ (also hardware-tested), `iec-nugget` ✅, `esp32-s3-devkitc-1` ✅. **`fujiloaf-rev0`** ✅ (only after the libarchive `-Os` entry above) — it had roughly **220 bytes of margin** at HEAD, so this session's ~460 bytes of source changes overflowed it whether the threshold was 128 (537 B over) or 512 (241 B over); the threshold was NOT what broke it, the absence of margin was. Two boards were already broken at HEAD for unrelated reasons: **`fujiapple-rev0`** does not define `ENABLE_CONSOLE`, but `CoreCommands.cpp` calls `console.requestExit()` outside the `#ifdef` (`'console' was not declared`), and **`freenove-esp32-cam`** builds a 4,351,704-byte app against a 2,949,120-byte partition. Note `build_type = debug` in `platformio.ini` defines `DEBUG` for every board, so all `Debug_printv` text is compiled in — a large part of why these images have no margin.

- **CORRECTION — the archive-over-HTTP failure is internal-DRAM exhaustion, not misalignment.** A later fresh run logged `pre-seek pos[0]` (correctly aligned) and still failed, with `first bytes[50 78 56 AD BA CD FF 3F 6C 69 62 2F 6D 65 61 74]` — a DRAM pointer (`0x3FFFCDBA`, little-endian) followed by the ASCII `lib/meat`, i.e. this firmware's own freed heap holding a `__FILE__` string from `Debug_printv`. The read did not return the wrong PART of the file; it returned a buffer that was never filled with HTTP data at all, while still reporting a positive byte count. Same visible symptom (no format bids, so `raw` used to swallow it), different mechanism. The stale-position bug below is real and independently reproduced, but it is NOT what caused the reported `unzipx` failure. See the internal-DRAM entry.
- **A cached stream's rewind was a silent no-op** (`lib/meatloaf/network/http.cpp/.h`, `lib/meatloaf/meatloaf.h`): seen on hardware as `Archive::open()` logging `pre-seek pos[524]` — the ImageBroker's cached `ArchiveMStream` still held an HTTP source 524 bytes in, and the `seek(0)` that should have rewound it did nothing. Two causes, both fixed: (1) **ONE `MeatHttpClient` is shared by every stream on a host:port**, so its `_position` belongs to whichever stream or reopen last touched it; `MeatHttpClient::seek()` decides whether a real re-request is needed by comparing the target against that counter and returns "already there" when they match, so a stale counter turns a rewind into a silent no-op. `HTTPMStream::seek()` now hands the client THIS stream's offset first (skipped in full mode, where `_position` indexes `_responseBuffer`/the JSON result rather than the HTTP body). (2) `MStream::seek(pos, mode)` committed `_position` **before** delegating, so `HTTPMStream::seek()` could never see where it had been — `HTTPMStream` now overrides the two-argument form to resolve the target without touching `_position`. The run that produced the log only succeeded by accident: its fast-path seek failed with `httpCode=-1` and fell through to the branch that genuinely re-requests from byte 0.
- **A refused seek left `_position` claiming a move that never happened** (`lib/meatloaf/meatloaf.h`): `MStream::seek(pos, mode)` assigned the target and returned the delegate's result without taking the assignment back on failure, so every later read was attributed to an offset the stream was not at — and callers that compare `position()` against where they asked to be were told a lie they could not detect. Now restores the previous offset when the delegate refuses. Covered by `test/native/test_mstream_seek/`.
- **`unzipx` exhausted internal DRAM on an archive with a deep tree** (`lib/console/Commands/VFSCommands.cpp`): `unzip_mkdirs()` re-walked every path prefix for every entry, and each `MFSOwner::File()` probes for a `.config` at every level above it — a cache that holds 64 entries and **clears wholesale on overflow**, which a deep archive triggers, so the probes fell through to real `fopen()`s on SD. Each needs a 512-byte `MALLOC_CAP_DMA` bounce buffer per sector plus a newlib FILE lock, both internal-DRAM-only, and the board was running at ~4-6 KB free internal: `sdmmc_read_sectors: not enough mem` followed by `abort()` inside `fopen()` (`lock_init_generic`). Segments created are now memoized per run — roughly 450 config probes down to ~50 for a 30-entry archive. **This reduces demand, it does not create headroom**: with the memoization in place the run got further (crashed at `HAUNT HOUSE` instead of `BLACKJACK 21`) and still died. Not covered by the native suite — `lib/console` is not compiled there.
- **What actually consumes the internal heap here**: `CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=512` means every allocation of 512 bytes or less is served from internal DRAM, and this workload is made of nothing else. With `CONFIG_FATFS_PER_FILE_CACHE=y` and `CONFIG_FATFS_MAX_LFN=255`, ONE open file on SD costs a 512-byte sector cache + a ~512-byte LFN working buffer + a newlib `FILE` + its lock — roughly 1.5 KB of internal DRAM — and `MFSOwner::File()` opens one per `.config` probe. Measured free internal during the run: `Low[1504]`-`Low[1684]`. mbedtls is NOT the culprit; `CONFIG_MBEDTLS_EXTERNAL_MEM_ALLOC=y` already puts TLS buffers in PSRAM. Fixed by dropping the threshold to 128 so those land in PSRAM — see the entry above for the result and the rollout. `CONFIG_FATFS_MAX_LFN` 255 → 128 is the next lever if more headroom is ever needed.
