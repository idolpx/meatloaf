// Pulls in the exact translation units the CSM read tests need, by
...
#include "../../../lib/meatloaf/media/tape/csm.cpp"

// Link-only stubs for symbols meatloaf.h/csm.cpp reference but these tests
// never call. Shared verbatim with the disk-write suite rather than copied.
// Note MFSOwner::File() aborts, so CSMMFile::rewindDirectory() and
// getNextFileInDir() are out of reach here - these tests drive CSMMStream,
// which is where the format knowledge lives.
